...
const parseArgs = (): ISCAAnalysisArgs => {
...
const runAnalysis = async (args: ISCAAnalysisArgs): Promise<void> => {
  const scanType = ScanType.SCA;
  const analysisService = AnalysisService.create(args.apiKey, args.apiURL);
  ...
  try {
    ...
    const { manifestFiles, hashManifests } = await analysisService.findManifestsAndHashableFiles({
      ...
      directoriesToExclude: args.directoriesToExclude,
      sourceCodePath: args.sourceCodePath,
      packageManagers: args.packageManagers ?? [],
      fileMatchType: args.fileMatchType,
    });
